...
```{bash}
$ docker push my-registry.com/mobyr
```
...
$ docker run -it --rm kuethe/gitbook:0.1
```
...
- __volumes:__ stored in a part of the host filesystem which is __managed by Docker__
- __bind mounts:__ may be stored __anywhere__ on the host system
- `tmpfs` __mounts:__ stored in the host system’s memory only, and are never written to the host system’s filesystem
